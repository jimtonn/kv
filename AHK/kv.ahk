...
Class kv
{
	...
	Set(key, value)
	{
		this.StdoutToVar_CreateProcess(this.PathToKv " " this.PathToStoreFile " set " key " " value, , , returnCode)
		...
		{
			throw Exception("kv: Store file not found: " this.PathToStoreFile)
		}
		else if returnCode = 4
		{
			throw Exception("kv: Key not found: " %key%)
		}
		else
		{
			throw Exception("kv: Unknown error occurred.")
		}
	}
	
	Get(key)
	{
		getResult := this.StdoutToVar_CreateProcess(this.PathToKv " " this.PathToStoreFile " get " key, , , returnCode)
